...
export default function shops({ products, categories, properties }) {
  ...
  useEffect(() => {
    localStorage.setItem('shopQuery', JSON.stringify(query.shop));
  }, [query.shop]);
  ...
  const handleFilterChange = (value, name) => {
    ...
    router.push({
      pathname: '/women/shops',
      query: updatedQuery,
    });
  };

  const clearFilters = () => {
    let defaultQuery = localStorage.getItem('shopQuery');
    if (defaultQuery) {
  ...
  return (
    <RootLayout>
      ...
      >
        "Women's shops" : <span>{query.shop && query.shop}</span>
      </h2>
      <FiltersContainer className="hidden md:flex px-10 justify-between py-5 bg-[#c6c4c4]">
        <Select
          value={query.shop || ''}
          onValueChange={(value) => {
            console.log('shop selected:', value); // Debugging log
            handleFilterChange(value, 'shop');
          }}
          name="shop"
        >
          ...
          <SelectContent>
            {properties.shop.map((shop, index) => (
